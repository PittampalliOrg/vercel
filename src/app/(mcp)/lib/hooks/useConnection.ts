...
// Get package version dynamically - no need for import from package.json
...
export function useConnection({
  ...
  const connect = async (_e?: unknown, retryCount: number = 0) => {
    try {
      if (!proxyServerUrl) {
        throw new Error("Invalid proxyServerUrl: URL is undefined or empty");
      }
      if (transportType === "sse" && !sseUrl) {
        throw new Error("Invalid sseUrl: URL is undefined or empty for SSE transport");
      }
