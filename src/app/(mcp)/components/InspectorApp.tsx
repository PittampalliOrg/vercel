...
const InspectorApp = () => {
  ...
  const [pendingSampleRequests, setPendingSampleRequests] = useState<Array<
    PendingRequest & {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // Check hash for initial tab selection
      const hash = window.location.hash.slice(1);
      if (hash && Object.keys(serverCapabilities || {}).includes(hash)) {
        setActiveTab(hash);
      } else if (serverCapabilities?.resources) {
        setActiveTab('resources');
      } else if (serverCapabilities?.prompts) {
        setActiveTab('prompts');
      } else if (serverCapabilities?.tools) {
        setActiveTab('tools');
      } else {
        setActiveTab('ping');
      }
