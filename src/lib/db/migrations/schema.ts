import { pgTable, uuid, varchar, foreignKey, timestamp, text, boolean, bigint, doublePrecision, numeric, json, unique, jsonb, primaryKey } from "drizzle-orm/pg-core"
...
export const apiEndpoints = pgTable("api_endpoints", {
	...
	id: bigint({ mode: "number" }).primaryKey().generatedAlwaysAsIdentity({ name: "api_endpoints_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
...
(table) => {
	return {
		apiEndpointsEndpointIdKey: unique("api_endpoints_endpoint_id_key").on(table.endpointId),
	}
});
